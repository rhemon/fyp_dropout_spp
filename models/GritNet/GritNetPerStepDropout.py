...
class GritNetPerStepDropout(GritNetNoDropout):
    """
    GritNet model with per step dropout.
    """
    
    def set_model(self, event_dim, embedding_dim, hidden_dim, target_size):
        """
        Initialize GritNet model with just PerStepBLSTM layer using per step
        dropout.
        ...
        blstm_layer = PerStepBLSTM(embedding_dim*2, hidden_dim, "Standard", self.drop_prob).to(self.device)
