...
class SimpleSentimentPerStepRNNDrop(SimpleSentimentNoDropout):
    """
    SimpleSentiment with RNNDrop.
    """
    
    def set_model(self, vocab_size, embedding_dim, hidden_dim, target_size, seq_len):
        """
        Initialize SimpleSentiment model with just PerStepBLSTM layer with RNNDrop.
        ...
        blstm_layer = PerStepBLSTM(embedding_dim, hidden_dim, "RNNDrop", self.drop_prob).to(self.device)
